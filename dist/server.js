...
import { loadTokens, loadComponents, loadLayout, loadUX, loadAccessibility, loadCharts, loadLandingComponents, loadEffects, getFullDesignSystem, listComponentNames, getComponentSpec, getTokenCategory, getLayoutPattern, getUXGuideline, listUXTopics, listDashboardComponentNames, getDashboardComponentSpec, listChartNames, getChartSpec, getChartCommonProps, getChartSharedComponents, listLandingComponentNames, getLandingComponentSpec, listEffectNames, getEffectSpec, loadPageTemplates, listPageTemplateNames, getPageTemplate, loadScreenshots, listScreenshotNames, getScreenshotBase64, } from './utils/data-loader.js';
// Tool definitions
const tools = [
    // === SERVE TOOLS ===
    {
...
function handleGetComponentSpec(args) {
...
function handleGetLayoutPatterns(args) {
    if (args.pattern) {
...
function handleGetUXGuidelines(args) {
    if (args.topic) {
...
function handleGetDashboardComponentSpec(args) {
...
function handleGetChartSpec(args) {
...
function handleListCharts() {
    ...
    return {
        ...
        charts: chartNames.map(name => ({
            name,
            description: charts[name]?.description || '',
        })),
        categories: {
...
function handleGetLandingComponentSpec(args) {
...
function handleGetEffectSpec(args) {
...
function handleGetPageTemplate(args) {
...
function handleGenerateTailwindClasses(args) {
    const spec = getComponentSpec(args.component);
    if (!spec) {
        return { error: `Component "${args.component}" not found.` };
    }
    ...
    if (spec.variants?.[variant]?.classes) {
        ...
        const v = spec.variants[variant];
        if (v.background)
            classes.push(`bg-${v.background}`);
        if (v.color)
            classes.push(`text-${v.color}`);
        if (v.hover)
            classes.push(`hover:bg-${v.hover}`);
    }
    // Size classes
...
function handleGenerateCSSVariables(args) {
    const tokens = loadTokens();
...
function handleGenerateComponentSkeleton(args) {
    const spec = getComponentSpec(args.component);
    if (!spec) {
        return { error: `Component "${args.component}" not found.` };
    }
...
function generateReactSkeleton(name, className, spec) {
    const classes = spec.baseStyles?.classes || spec.properties?.classes || className;
    if (spec.components) {
...
function generateVueSkeleton(name, className, spec) {
    const classes = spec.baseStyles?.classes || spec.properties?.classes || className;
    return `<!-- ${name}.vue -->
<template>
...
function generateSvelteSkeleton(name, className, spec) {
    const classes = spec.baseStyles?.classes || spec.properties?.classes || className;
    return `<!-- ${name}.svelte -->
<script lang="ts">
...
function handleValidateColors(args) {
    const tokens = loadTokens();
    const allColors = {};
    // Collect all color values
    for (const [key, value] of Object.entries(tokens.colors.light)) {
        allColors[value.value.toLowerCase()] = key;
    }
    for (const [key, value] of Object.entries(tokens.colors.semantic)) {
        allColors[value.value.toLowerCase()] = key;
    }
...
function handleValidateSpacing(args) {
    const tokens = loadTokens();
...
function handleValidateComponent(args) {
    const tokens = loadTokens();
...
function handleAuditDesign(args) {
    const colorResult = handleValidateComponent({ description: args.description });
    const tokens = loadTokens();
    const accessibility = loadAccessibility();
    const categories = {
        colors: { score: 100, issues: [], suggestions: [] },
        typography: { score: 100, issues: [], suggestions: [] },
        spacing: { score: 100, issues: [], suggestions: [] },
        accessibility: { score: 100, issues: [], suggestions: [] },
        responsiveness: { score: 100, issues: [], suggestions: [] },
        components: { score: 100, issues: [], suggestions: [] },
    };
    const description = args.description.toLowerCase();
    // Color audit
        ...
        try {
            ...
            switch (name) {
                ...
                case 'get_dashboard_component_spec':
                    result = handleGetDashboardComponentSpec(args);
                    break;
                case 'list_dashboard_components':
                    result = handleListDashboardComponents();
                    break;
                case 'get_chart_spec':
                    result = handleGetChartSpec(args);
                    break;
                case 'list_charts':
                ...
                case 'get_page_template':
                    result = handleGetPageTemplate(args);
                    break;
                ...
                // Screenshot tools
                case 'get_screenshot': {
                    const screenshotData = getScreenshotBase64(args.page_name);
                    if (!screenshotData) {
                        const available = listScreenshotNames();
                        return {
                            content: [{
                                    type: 'text',
                                    text: JSON.stringify({ error: `Screenshot "${args.page_name}" not found. Available: ${available.join(', ')}` }, null, 2),
                                }],
                            isError: true,
                        };
                    }
                ...
                // Generate tools
                case 'generate_css':
                    result = handleGenerateCSS(args);
                    break;
                case 'generate_tailwind_classes':
                    result = handleGenerateTailwindClasses(args);
                    break;
                case 'generate_css_variables':
                    result = handleGenerateCSSVariables(args);
                    break;
                case 'generate_component_skeleton':
                    result = handleGenerateComponentSkeleton(args);
                    break;
                // Validate tools
                case 'validate_colors':
                    result = handleValidateColors(args);
                    break;
                case 'validate_spacing':
                    result = handleValidateSpacing(args);
                    break;
                case 'validate_component':
                    result = handleValidateComponent(args);
                    break;
                case 'audit_design':
                    result = handleAuditDesign(args);
                    break;
                default:
